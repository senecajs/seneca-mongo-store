...
function fixquery(q, plugin_opts) {
  if (q.native$) {
  ...
  for (const qp in q) {
    ...
    if (is_mongo_qualifier(qp) && should_strip_mongo_qualifiers(plugin_opts)) {
      continue
    }
...
function should_strip_mongo_qualifiers(plugin_opts) {
  return false == plugin_opts.mongo_operator_shortcut
}
